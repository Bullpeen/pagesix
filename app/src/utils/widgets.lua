...
-- patches the score span (id `<kind>-score-<id>`) in place via SSE. No classes;
-- styling targets the `data-votes` / `data-vote` attributes.
...
function Widgets.votes(kind, id, score, csrf)
	...
	local function arrow(dir, glyph)
		...
		return (
			...
			.. '<button data-vote="%s" type="submit" aria-label="%svote">%s</button></form>'
		):format(url, url, csrf, dir, dir, glyph)
	end
	return ('<div data-votes>%s<span id="%s-score-%s">%s</span>%s</div>'):format(
		arrow("up", "&#9650;"),
		kind,
		id,
		score,
